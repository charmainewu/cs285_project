...
class MultiHeadAttentionNoDual(nn.Module):
    def __init__(
            self,
            n_heads,
            input_dim,
        ...
        super(MultiHeadAttentionNoDual, self).__init__()
    ...
    def forward(self, h_node_in, h_pos_in): #input (NFEs, PFEs)
        ...
        heads_node_1 = torch.matmul(attn1, V_node[0]) # self-attn
        heads_node_2 = torch.matmul(attn1, V_node[1]) # cross-aspect ref attn
        
        heads_pos_1 = torch.matmul(attn2, V_pos[0]) # cross-aspect ref attn
        heads_pos_2 = torch.matmul(attn2, V_pos[1]) # self-attn
...
# implements the DAC encoder
class MultiHeadEncoderNoDual(nn.Module):
    ...
    def __init__(
        ...
        super(MultiHeadEncoderNoDual, self).__init__()
        
        self.MHA_sublayer = MultiHeadAttentionsubLayerNoDual(
                        n_heads,
                        embed_dim,
                        feed_forward_hidden,
                        normalization=normalization,
...
# implements the DAC encoder (DAC-Att sublayer)   
class MultiHeadAttentionsubLayerNoDual(nn.Module):
    ...
    def __init__(
        ...
        super(MultiHeadAttentionsubLayerNoDual, self).__init__()
        
        self.MHA = MultiHeadAttentionNoDual(
                    n_heads,
                    input_dim=embed_dim,
                    embed_dim=embed_dim
                )
...
class Decoder_LSTM(nn.Module):
    ...
    def forward(self, embedded, hidden, cell):
        # input = [batch_size]
        ...
        embedded_t = torch.cat(embedded, -1).detach()
        # print('embed size', embedded_t.shape)
        hidden_t = torch.cat(hidden, -1).detach()
        # print('hidden size', hidden_t.shape)
        cell_t = torch.cat(cell, -1).detach()
        # print('cell size', cell_t.shape)

        embedded_t = embedded_t.transpose(0,1)
        output, (hidden, cell) = self.rnn(embedded_t, (hidden_t, cell_t))
        # outputs = outputs.transpose(0,1)
