from torch import nn
import torch
from nets.graph_layers import MultiHeadEncoder, MultiHeadDecoder, EmbeddingNet, Encoder_LSTM, Encoder_GRU, Decoder_GRU, Decoder_LSTM, MultiHeadEncoderNoDual
from torch.distributions import Categorical
import torch.nn.functional as F

class mySequential(nn.Sequential):
    def forward(self, *inputs):
        for module in self._modules.values():
            if type(inputs) == tuple:
                inputs = module(*inputs)
            else:
                inputs = module(inputs)
        return inputs

class Actor(nn.Module):
    def __init__(self,
                 exp_dual,
                 exp_encoder,
                 exp_decoder,
                 problem_name,
                 embedding_dim,
                 hidden_dim,
                 n_heads_actor,
                 n_heads_decoder,
                 n_layers,
                 normalization,
                 v_range,
                 seq_length,
                 ):
        super(Actor, self).__init__()
        self.exp_dual = exp_dual
        self.exp_encoder = exp_encoder
        self.exp_decoder = exp_decoder
        self.problem_name = problem_name
        self.embedding_dim = embedding_dim
        self.hidden_dim = hidden_dim
        self.n_heads_actor = n_heads_actor
        self.n_heads_decoder = n_heads_decoder        
        self.n_layers = n_layers
        self.normalization = normalization
        self.range = v_range
        self.seq_length = seq_length
        self.drop_out = 0.05
        
        # see Appendix D
        if problem_name == 'tsp':
            self.node_dim = 2
        elif problem_name == 'cvrp':
            self.node_dim = 7
        else:            
            assert False, "Unsupported problem: {}".format(self.problem.NAME)        

        # build DACT model
        self.embedder = EmbeddingNet(
                            self.node_dim,
                            self.embedding_dim,
                            self.seq_length)
        
        self.encoder_base_dual = mySequential(*(
                MultiHeadEncoder(self.n_heads_actor, 
                                self.embedding_dim, 
                                self.hidden_dim, 
                                self.normalization,)
            for _ in range(self.n_layers))) # stack L layers

        self.encoder_base_nodual = mySequential(*(
                MultiHeadEncoderNoDual(self.n_heads_actor, 
                                self.embedding_dim, 
                                self.hidden_dim, 
                                self.normalization,)
            for _ in range(self.n_layers))) # stack L layers

        self.encoder_lstm = Encoder_LSTM(self.embedding_dim, self.embedding_dim, self.hidden_dim, self.n_layers, self.drop_out)
        self.encoder_gru = Encoder_GRU(self.embedding_dim, self.embedding_dim, self.hidden_dim, self.n_layers, self.drop_out)

        self.decoder_base = MultiHeadDecoder(n_heads = self.n_heads_decoder,
                                        input_dim = self.embedding_dim, 
                                        embed_dim = self.embedding_dim)
        self.decoder_lstm = Decoder_LSTM(self.seq_length, self.embedding_dim*2, self.hidden_dim*2, self.n_layers, self.drop_out)
        self.decoder_gru = Decoder_GRU(self.seq_length, self.embedding_dim*2, self.hidden_dim*2, self.n_layers, self.drop_out)

        print(self.get_parameter_number())

    def get_parameter_number(self):
        total_num = sum(p.numel() for p in self.parameters())
        trainable_num = sum(p.numel() for p in self.parameters() if p.requires_grad)
        return {'Total': total_num, 'Trainable': trainable_num}

    def forward(self, problem, x_in, solution, exchange, do_sample = False, fixed_action = None, require_entropy = False, to_critic = False, only_critic  = False):

        bs, gs, in_d = x_in.size()
        
        if problem.NAME == 'cvrp':
            
            # prepare the 7-dim features x_in
            argsort = solution.argsort()
            loc = x_in[:,:,:2]            
            post = loc.gather(1, solution.view(bs, -1, 1).expand_as(loc))
            pre = loc.gather(1, argsort.view(bs, -1, 1).expand_as(loc))
            post = torch.norm(post - loc, 2, -1, True)
            pre = torch.norm(pre - loc, 2, -1, True)
            
            x_in = torch.cat((loc, pre, post, x_in[:,:,-1:]), -1)
            del post, pre, argsort
            
            # get feasibility masks for current state
            mask_table, contex, to_actor = problem.get_swap_mask(solution, x_in)
            mask_table = mask_table.cpu()
            
            # concate the 7-dim features x_in
            x_in = torch.cat((x_in[:,:,:4], to_actor), -1)
            del to_actor
            contex = contex % 1000 // 1
            
            # pass through embedder to get embeddings
            NFE, PFE, visited_time = self.embedder(x_in, solution, visited_time = contex)
        
        elif problem.NAME == 'tsp':
            # pass through embedder
            #print('x_in size', x_in.shape)
            NFE, PFE, visited_time = self.embedder(x_in, solution, None)
            
            # mask infeasible solutions
            mask_table = problem.get_swap_mask(visited_time).expand(bs, gs, gs).cpu()
        
        else: 
            raise NotImplementedError()
        
        del visited_time


        if self.exp_dual == True:
            if self.exp_encoder=='attention':
                h_em, g_em = self.encoder_base_dual(NFE, PFE)
            else:
                h_em,h_hidden,h_cell = self.encoder_lstm(NFE)
                g_em,g_hidden,g_cell = self.encoder_lstm(PFE)
            if only_critic:
                return (h_em, g_em)
            if self.exp_decoder=='attention':
                compatibility = torch.tanh(self.decoder_base(h_em, g_em, None)) * self.range
                compatibility[mask_table] = -1e20
                del mask_table
                if exchange is not None:
                    compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
                    compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
                im = compatibility.view(bs, -1)
            else:
                prediction,_,_ = self.decoder_lstm((h_em, g_em),(h_hidden,g_hidden),(h_cell,g_cell))
                compatibility = torch.tanh(prediction) * self.range
                compatibility[mask_table] = -1e20
                del mask_table
                if exchange is not None:        
                    compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
                    compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
                im = compatibility.contiguous().view(bs, -1)
        else:
            if self.exp_encoder=='attention':
                h_em, g_em = self.encoder_base_nodual(NFE, PFE)
            else:
                h_em,h_hidden,h_cell = self.encoder_lstm(NFE)
                g_em,g_hidden,g_cell = self.encoder_lstm(PFE)
            if only_critic:
                return (h_em, g_em)
            if self.exp_decoder=='attention':
                compatibility = torch.tanh(self.decoder_base(h_em, g_em, None)) * self.range
                compatibility[mask_table] = -1e20
                del mask_table
                if exchange is not None:
                    compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
                    compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
                im = compatibility.view(bs, -1)
            else:
                prediction,_,_ = self.decoder_lstm((h_em, g_em),(h_hidden,g_hidden),(h_cell,g_cell))
                compatibility = torch.tanh(prediction) * self.range
                compatibility[mask_table] = -1e20
                del mask_table
                if exchange is not None:        
                    compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
                    compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
                im = compatibility.contiguous().view(bs, -1)            


        # if self.exp_name == 0:
        #     h_em, g_em = self.encoder_base(NFE, PFE)
        #     if only_critic:
        #         return (h_em, g_em)
        #     if self.exp_combine == 0:
        #         compatibility = torch.tanh(self.decoder_base(h_em, g_em, None)) * self.range
        #     else:
        #         compatibility = torch.tanh(self.decoder_base(h_em, g_em, None)) * self.range
        #     compatibility[mask_table] = -1e20
        #     del mask_table
        #     if exchange is not None:
        #         compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
        #         compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
        #     im = compatibility.view(bs, -1)
        
        # if self.exp_name == 1:
        #     h_em,h_hidden,h_cell = self.encoder_lstm(NFE)
        #     g_em,g_hidden,g_cell = self.encoder_lstm(PFE)
        #     if only_critic:
        #         return (h_em, g_em)
        #     compatibility = torch.tanh(self.decoder_base(h_em, g_em, None)) * self.range
        #     compatibility[mask_table] = -1e20
        #     del mask_table
        #     if exchange is not None:        
        #         compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
        #         compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
        #     im = compatibility.view(bs, -1)
        
        # if self.exp_name == 2:
        #     h_em,h_hidden,h_cell = self.encoder_lstm(NFE)
        #     g_em,g_hidden,g_cell = self.encoder_lstm(PFE)
        #     if only_critic:
        #         return (h_em, g_em)
        #     prediction,_,_ = self.decoder_lstm((h_em, g_em),(h_hidden,g_hidden),(h_cell,g_cell))
        #     compatibility = torch.tanh(prediction) * self.range
        #     compatibility[mask_table] = -1e20
        #     del mask_table
        #     if exchange is not None:        
        #         compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
        #         compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
        #     im = compatibility.contiguous().view(bs, -1)
        
        # if self.exp_name == 3:
        #     h_em,h_hidden,h_cell = self.encoder_gru(NFE)
        #     g_em,g_hidden,g_cell = self.encoder_gru(PFE)
        #     if only_critic:
        #         return (h_em, g_em)
        #     compatibility = torch.tanh(self.decoder_base(h_em, g_em, None)) * self.range
        #     compatibility[mask_table] = -1e20
        #     del mask_table
        #     if exchange is not None:        
        #         compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
        #         compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
        #     im = compatibility.view(bs, -1)
        
        # if self.exp_name == 4:
        #     h_em,h_hidden,h_cell = self.encoder_gru(NFE)
        #     g_em,g_hidden,g_cell = self.encoder_gru(PFE)
        #     if only_critic:
        #         return (h_em, g_em)
        #     prediction,_,_ = self.decoder_gru((h_em, g_em),(h_hidden,g_hidden),(h_cell,g_cell))
        #     compatibility = torch.tanh(prediction) * self.range
        #     compatibility[mask_table] = -1e20
        #     del mask_table
        #     if exchange is not None:        
        #         compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
        #         compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
        #     im = compatibility.contiguous().view(bs, -1)
        
        # #print('seq_len',self.seq_length)
        # # # pass through DAC encoder
        # # h_em, g_em = self.encoder(NFE, PFE)
        # # print('NFE, PEF: ',NFE.shape, PFE.shape)
        # # print('h_em,g_em size: ',h_em.shape,g_em.shape)
        # # print('h_hidden size: ',h_hidden.shape)
        # # print('h_cell: ', h_cell.shape)
        # h_em,h_hidden,h_cell = self.encoder(NFE)
        # g_em,g_hidden,g_cell = self.encoder(PFE)

        # # # share embeddings to critic net
        # if only_critic:
        #     #print('this is a cat')
        #     return (h_em, g_em)
        
        # # pass through DAC decoder
        # # compatibility = torch.tanh(self.decoder(h_em, g_em, None)) * self.range
        # # xx = self.decoder(h_em, g_em, None)
        # # print('decoder:',xx.shape)
        # # print('compatibility:',compatibility.shape)
        # # print('decoder begin')
        # prediction,_,_ = self.decoder((h_em, g_em),(h_hidden,g_hidden),(h_cell,g_cell))
        # compatibility = torch.tanh(prediction) * self.range
        # # print('decoder:',prediction.shape)
        # # print('compatibility:',compatibility.shape)

        # # perform masking
        # compatibility[mask_table] = -1e20
        
        # del mask_table

        # # mask the last action to aviod loops
        # if exchange is not None:        
        #     compatibility[torch.arange(bs), exchange[:,0], exchange[:,1]] = -1e20
        #     compatibility[torch.arange(bs), exchange[:,1], exchange[:,0]] = -1e20            
        
        # #reshape our tables
        # im = compatibility.contiguous().view(bs, -1)

        # softmax
        log_likelihood = F.log_softmax(im,dim = -1)
        M_table = F.softmax(im,dim = -1)
        
        # fixed action for PPO training if needed
        if fixed_action is not None:
            row_selected = fixed_action[:,0]
            col_selected = fixed_action[:,1]
            pair_index = row_selected * gs + col_selected
            pair_index = pair_index.view(-1,1)
            pair = fixed_action
            
        else:
            # sample one action
            if do_sample: 
                pair_index = M_table.multinomial(1)
            else:
                pair_index = M_table.max(-1)[1].view(-1,1)
        
            # from action (selected node pair)
            row_selected = pair_index // gs 
            col_selected = pair_index % gs
            pair = torch.cat((row_selected, col_selected),-1)  # pair: no_head bs, 2
 
        selected_log_likelihood = log_likelihood.gather(1, pair_index)
    
        if require_entropy:
            
            dist = Categorical(M_table, validate_args=False) # for logging only
            entropy = dist.entropy() # for logging only
            
            out = (pair,
                   selected_log_likelihood.squeeze(),
                   (h_em, g_em) if to_critic else None,
                   entropy)
        else:
            out = (pair,
                   selected_log_likelihood.squeeze(),
                   (h_em, g_em) if to_critic else None)
            
        return out
